...
// .ogg bên trong: mục đích của hash này là XÁC THỰC file zip tải về có
...
// NỘI DUNG BÀI đã đổi, so khớp qua tên file). Từng file .ogg bên trong zip
// vẫn phải tự đặt tên đúng quy ước "{type}_{itemId}_{contentHash}.ogg" của
// TtsAudioCache để sau khi giải nén, cache tự nhận ra — đây là việc của bên
// ĐÓNG GÓI zip (server/pipeline build gói), không phải việc của app.
...
// bắt buộc app phải dùng ngay ở bước này (đã có sha256 làm cơ chế phát hiện
// bản mới chính, xem TtsRemotePackDownloader.checkForUpdate()).
...
// được thiết kế), packs ở đây có thể chứa nhiều sid khác nhau của CÙNG 1
// readingId đó (mỗi giọng người dùng có thể chọn là 1 pack riêng).
