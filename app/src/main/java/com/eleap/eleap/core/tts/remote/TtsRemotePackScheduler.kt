...
//
// Nơi DUY NHẤT gọi WorkManager để enqueue TtsRemotePackWorker — mỗi
// (readingId, sid) có 1 tên unique work RIÊNG (không phải 1 tên duy nhất
// cho toàn app) — vì đây là việc tải 1 gói CỤ THỂ, nhiều gói khác nhau có
...
object TtsRemotePackScheduler {
    ...
    // dùng mở 1 bài đọc (biết ngay readingId + sid đang chọn). KEEP — nếu đã
    // có lượt tải đang chạy/đang chờ mạng cho ĐÚNG cặp này, không tạo bản
    // sao chạy song song; nếu lượt trước đã XONG (thành công hay thất bại
    // đều là "xong"), KEEP vẫn cho enqueue lại bình thường.
